...
function changeComment($idComment, $comment)
{
    ...
    if ($affectedComment === false) {
        ...
        header('Location: ./index.php?action=billet&id=' . $idComment['id']);
    }

    require('view/PostView.php');
}
function idk ($billetId)
{
    $getIdComment = new CommentManager();
    $commentId = $getIdComment->getComment($commentId);
    
    $getBilletFromCommentId = new BilletManager();
    $billetId = $getBilletFromCommentId->getBilletFromCommentId($commentId);
    if ($$billetId === false){
        throw new exception('Je ne sais pas ce que je fais !!!!!!!!');
    }
    else {
        header('Location: ./index.php?action=billet&billetId=' . $billetId['id'] . '&id=' . $idComment['id']);
    }
    require('view/postView.php');
}